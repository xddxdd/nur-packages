...
- **跟踪 git HEAD 且有依赖哈希的 Go 包用 nix-update-script --version branch**：`unstableGitUpdater`（update-source-version）只处理 version/rev/src hash，不更新 vendorHash，上游 go.mod 变了会静默过期（oidc-tester-app 曾因此构建失败）。若包不需要 `hardcodeZeroVersion`/`tagPrefix` 等 unstableGitUpdater 特有参数，直接用 `nix-update-script { extraArgs = [ "--version" "branch" ]; }`：nix-update 的 branch 模式从分支 atom feed 取 HEAD 日期生成 `<release或0>-unstable-<日期>` 版本并更新 rev/src hash，同时在同一次运行里预取 goModules 刷新 vendorHash（参考 `pkgs/uncategorized/oidc-tester-app`）。注意 atom feed 的日期取 entry 的 `updated` 字段，与 unstableGitUpdater 的 committer date（`%cs`）可能相差一天；仓库存在无关 tag 时 branch 模式会用 tag 数字作版本前缀，此时仍需 unstableGitUpdater + 自定义脚本组合
