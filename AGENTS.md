...
- **pythonMetadataCheckPhase 与 pname 不一致**：新版 nixpkgs 的 `pythonMetadataCheckPhase` 钩子用 `importlib.metadata.version($pname)` 校验已安装包元数据，要求派生 `pname` 与上游 PyPI 分发名（PEP 503 归一化）一致。本仓库 `pname` 来自 nvfetcher 配置段名（更具描述性），常与上游包名不同（如 `data-recorder` vs `DataRecorder` 归一化为 `datarecorder`、`email-oauth2-proxy` vs `emailproxy`、`runpod-python` vs `runpod`），导致 `PackageNotFoundError` 构建失败。修复方法：在 `buildPythonPackage` 中设置 `dontCheckPythonMetadata = true;`。
- **nixpkgs 依赖被 setuptools 上界约束打破**：当 nixpkgs 的某个 Python 依赖（如 `mfusepy`）在 `[build-system] requires` 中钉死 `setuptools < 83`，而 nixpkgs 已升级到 setuptools 83，构建期会报 `Unmet dependencies`。可在 `pkgs/python-packages/default.nix` 的 `self` 集合中用 `pkgs.python3Packages.<pkg>.overridePythonAttrs` 放宽约束：将 `postPatch` 设为 `(old.postPatch or "") +` 追加一段 `substituteInPlace pyproject.toml` 把 `setuptools >= 61, < 83` 改为 `setuptools >= 61`（务必拼接原有 `postPatch`，避免丢失既有补丁）。
